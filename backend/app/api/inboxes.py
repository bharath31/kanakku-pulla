...
async def create_inbox(data: InboxCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    inbox = await create_agentmail_inbox(db, data.username)
    return inbox


@router.get("/", response_model=list[InboxResponse])
def list_inboxes(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
