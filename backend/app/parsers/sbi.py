...
class SBIParser(BaseBankParser):
    ...
    def generate_pdf_password(self, holder_name: str, dob: date | None) -> str | None:
        if not dob or not holder_name:
            return None
        # SBI: first 4 chars UPPERCASE + DDMM
    ...
    def _try_parse_txn_line(self, line: str) -> ParsedTransaction | None:
        ...
        return ParsedTransaction(
            ...
            amount=amount,
            merchant_name=re.sub(r"^(POS|ECOM|UPI)\s+", "", description, flags=re.IGNORECASE).strip(),
            is_fee=is_fee,
            fee_type=fee_type,
